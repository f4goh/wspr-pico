...
public:
    ...
    void setManchesterTempo(duration _tempo);
    void setManchesterTxMode(mode _txMode);
    void sendManchester(const char *txt);
    
    void sendManchesterLoc(const char *call, byte ssid, char symbol, float latitude, float longitude);
...
private:
    ...
    void sendManchesterByte(byte val);
    void sendManchesterFloat(float angle, byte *crc);
    void sendManchesterVaricode(char c);
